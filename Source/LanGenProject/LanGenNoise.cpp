...
#include "LanGenNoise.h"

void ULanGenNoise::ResetSeed()
{
	p.Empty();
	p = P_BASE;
	p.Append(P_BASE);
...
void ULanGenNoise::InitSeed(int32 in)
{
	seed = in;
	p.Empty();
	p = P_BASE;
...
float ULanGenNoise::PerlinNoise3D(FVector location, int n, float lacunarity, float persistence, float in)
{
...
float ULanGenNoise::SimplexNoise3D(FVector location, int n, float lacunarity, float persistence, float in)
{
    // change frequency by changing x y z input value
    float multiplier = FMath::Pow(lacunarity, n);
    float x = location.X * multiplier;
...
float ULanGenNoise::Fade(float t) { return t * t * t * (t * (t * 6 - 15) + 10); }
float ULanGenNoise::Lerp(float t, float a, float b) { return a + t * (b - a); }
float ULanGenNoise::Grad(int hash, float x, float y, float z)
{
    int h = hash & 15;
